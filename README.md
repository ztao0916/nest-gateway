...
```
如果未提供所需的环境变量或它们不符合某些验证规则，则在应用启动期间抛出异常是标准做法
```

所以需要对环境变量做校验,传送门:[架构验证](https://nest.nodejs.cn/techniques/configuration#schema-validation)

初步判断: 环境变量可有可无,影响不大,可以不做
